...
if($_GET['act']=='update'){
	$tid=$_GET['tid'];
	$task=C::t('#gfarm#gfarm_achieve_task')->fetch($tid);
	if($_GET['formhash']==formhash()){
		...
		if(empty($_GET['task_name'])){
			...
			$countname=C::t('#gfarm#gfarm_achieve_task')->count_all_data(" and task_name='".$_GET['task_name']."'");
			if(empty($tid)){
				if(!empty($countname)){
					echo "<script language='javascript'>";
		...
		if(empty($tid)){		
			...
			C::t('#gfarm#gfarm_achieve_task')->insert($updatearray);
		}else{				
			C::t('#gfarm#gfarm_achieve_task')->update($tid,$updatearray);			
		}
		echo "<script>";
	...
	$taskways=C::t('#gfarm#gfarm_task_way')->fetch_all_data(' order by id');
	...
	include template('gfarm:cp/task/newachievetask');
	exit;
}
$text='';
...
$num=C::t('#gfarm#gfarm_achieve_task')->count_all_data($str);		
$taskInfos=C::t('#gfarm#gfarm_achieve_task')->fetch_all_data($str,$start,$perpage);
...
include template('gfarm:cp/task/achievetask');
