...
$formulalogInfos=C::t('#gfarm#gfarm_formula_log')->fetch_all_data($str,$start,$perpage);
foreach ($formulalogInfos as $key=> $value) {
	$users[$key]=C::t('#gfarm#gfarm_member')->fetch_by_uid($value['uid']);
}
$num=C::t('#gfarm#gfarm_formula_log')->count_all_data($str);
if ($num == 0){
	$maxpage = 1;
}else {
...
include template('gfarm:cp/formulagoods/formulalog');
