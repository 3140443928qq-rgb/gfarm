...
if(!empty($start_time)){
	$str.=' and a.create_time>='.$start_time;
	$text.='&start_time='.$_GET['start_time'];
}
if(!empty($end_time)){
	$str.=' and a.create_time<='.$end_time;
...
$str.=' order by a.create_time '.$ordertype1;
$text.='&ordertype='.$ordertype1;
$taskInfos=C::t('#gfarm#gfarm_member_act')->fetch_all_data($str,$start,$perpage);
$num=C::t('#gfarm#gfarm_member_act')->count_all_data($str);	
foreach ($taskInfos as $key=>$value) {
	$crops[$key]=C::t('#gfarm#gfarm_goods')->fetch($value['target_object']);
...
include template('gfarm:cp/task/actlog');
