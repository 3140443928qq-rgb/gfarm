...
$landlogInfos=C::t('#gfarm#gfarm_land_log')->fetch_all_data($str,$start,$perpage);
foreach ($landlogInfos as $key=>$value) {
	$users[$key]=C::t('#gfarm#gfarm_member')->fetch_by_uid($value['uid']);
	$lands[$key]=C::t('#gfarm#gfarm_member_land')->fetch($value['target_land']);
}
...
include template('gfarm:cp/user/landlog');
