...
class table_gfarm_exchange_log extends discuz_table
{
	public function __construct() {

		$this->_table = 'gfarm_exchange_log';
		$this->_pk    = 'id';
	...
	public function sum_money($where=''){
		$str='select sum(money) allmoney from %t where 1 %i group by uid';
		return DB::result_first($str, array($this->_table,$where));
	}
	public function sum_exnum($where=''){
		$str='select sum(exnum) from %t where 1 %i group by uid';
		return DB::result_first($str, array($this->_table,$where));
	}
}
